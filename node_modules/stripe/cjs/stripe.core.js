...
class Stripe {
    static initialize(platformFunctions, requestSenderFactory = defaultRequestSenderFactory) {
        ...
        Stripe.aiAgent = env ? (0, utils_js_1.detectAIAgent)(env) : '';
    ...
    constructor(key, config = {}) {
        ...
        this._api = {
            ...
            maxNetworkRetries: (0, utils_js_1.validateInteger)('maxNetworkRetries', props.maxNetworkRetries, this._platformFunctions.getDefaultMaxNetworkRetries()),
            agent: agent,
    ...
    _buildEventNotification(parsed) {
        ...
        if (parsed.context) {
            parsed.context = StripeContext_js_1.StripeContext.parse(parsed.context);
        }
        parsed.fetchEvent = () => {
            return this._requestSender._rawRequest('GET', `/v2/core/events/${parsed.id}`, undefined, {
        ...
        parsed.fetchRelatedObject = () => {
            ...
            return this._requestSender._rawRequest('GET', parsed.related_object.url, undefined, {
                stripeContext: parsed.context,
                headers: {
                    'Stripe-Request-Trigger': `event=${parsed.id}`,
                },
    ...
    // this return type is ignored?? picks up types from `types/index.d.ts` instead
        ...
        return this._buildEventNotification((0, utils_js_1.parsePayload)(payload));
    }
    ...
    parseEventNotificationWithoutVerification(payload) {
        return this._buildEventNotification((0, utils_js_1.maybeExtractFromCloudProviderEnvelope)(payload));
    }
}
 ...
 */
Stripe.MAJOR_API_VERSION = apiVersion_js_1.ApiMajorVersion;
Stripe.aiAgent = '';
Stripe.AI_AGENT = '';
Stripe.USER_AGENT = {
    bindings_version: Stripe.PACKAGE_VERSION,
...
Stripe.Decimal = shared_js_1.Decimal;
