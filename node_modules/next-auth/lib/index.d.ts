...
export declare function initAuth(config: NextAuthConfig | ((request: NextRequest | undefined) => Awaitable<NextAuthConfig>), onLazyLoad?: (config: NextAuthConfig) => void): ((...args: WithAuthArgs) => Promise<Response | Session | ((...args: Parameters<NextAuthMiddleware | AppRouteHandlerFn>) => Promise<Response>) | null>) | ((...args: WithAuthArgs) => Promise<Response> | Promise<Session | null> | ((...args: Parameters<NextAuthMiddleware | AppRouteHandlerFn>) => Promise<Response>));
