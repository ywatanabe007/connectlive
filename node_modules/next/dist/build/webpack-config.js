...
const getCacheDirectories = process.env.NEXT_RSPACK ? (configs)=>{
...
async function getBaseWebpackConfig(dir, { buildId, encryptionKey, config, compilerType, dev = false, entrypoints, deferredEntrypoints, isDevFallback = false, pagesDir, rewrites, originalRewrites, originalRedirects, runWebpackSpan, appDir, middlewareMatchers, noMangling, jsConfig, jsConfigPath, resolvedBaseUrl, supportedBrowsers, clientRouterFilters, fetchCacheKeyPrefix, isCompileMode, previewProps }) {
    ...
    const CompleteRuntimePlugin = isRspack ? bundler.ForceCompleteRuntimePlugin : _forcecompleteruntime.default;
    ...
    let webpackConfig = {
        ...
        output: {
            ...
            ...config.outputHashSalt ? {
                hashSalt: config.outputHashSalt
            } : {}
        },
        performance: false,
        ...
        plugins: [
            ...
            !dev && isClient && (()=>{
                const mode = (0, _configshared.resolveCssChunkingMode)(config.experimental.cssChunking);
                if (mode !== 'loose' && mode !== 'strict') return false;
                const strict = mode === 'strict';
                return isRspack ? new ((0, _getrspack.getRspackCore)()).experiments.CssChunkingPlugin({
                    strict,
                    nextjs: true
                }) : new _csschunkingplugin.CssChunkingPlugin(strict);
            })(),
            telemetryPlugin,
    ...
    const cache = {
        ...
        version: `${__dirname}|${"16.3.1"}|${configVars}`,
    ...
    if (isRspack) {
        ...
        webpack5Config.cache = {
            ...
            storage: {
                type: 'filesystem',
                directory: _path.default.join(distDir, 'cache', 'rspack', rspackCacheKey)
            },
            version: `${__dirname}|${"16.3.1"}|${configVars}|${rspackCacheKey}`
        };
    }
    ...
    if (isRspack && 'CI' in process.env) {
        ...
        } else if (stats && typeof stats === 'object' && !Array.isArray(stats) && stats.colors === undefined) {
            stats.colors = false;
        }
    }
...
function getNextRootParamsRules({ isClient, appDir, pageExtensions }) {
