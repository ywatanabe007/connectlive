...
import { CONFIG_FILES, PHASE_DEVELOPMENT_SERVER, PHASE_EXPORT, PHASE_PRODUCTION_BUILD, PHASE_PRODUCTION_SERVER, PHASE_INFO } from '../shared/lib/constants';
...
import { findRootDirAndLockFiles, warnDuplicatedLockFiles, warnRootBoundary } from '../lib/find-root';
 ...
 */ function assignDefaultsAndValidate(dir, userConfig, silent, phase) {
    ...
    if (result.cacheLife) {
        ...
        for (const [profileName, profile] of Object.entries(userCacheLifeProfiles)){
            cacheLifeProfiles[profileName] = validateAndNormalizeCacheLifeProfile(profile, {
                kind: 'config',
                profileName
            });
        }
    ...
    if (result.experimental.useCache !== undefined) {
        ...
        if (!silent) {
            if (result.cacheComponents) {
                Log.warnOnce(`\`experimental.useCache\` is no longer needed, because \`cacheComponents\` already enables the \`"use cache"\` directive. You can remove it from ${configFileName}.`);
            } else {
                Log.warnOnce(`\`experimental.useCache\` is deprecated. Please use the top-level \`cacheComponents\` option instead in ${configFileName}.`);
            }
        }
    }
...
export default async function loadConfig(phase, dir, opts = {}) {
    ...
    if (!meta.cacheHit && logTiming) {
        ...
        Log.event(`Running ${meta.configFileName ?? 'next.config'} took ${hrtimeBigIntDurationToString(durationNanos)}`);
    }
    return config;
}
async function loadConfigImpl(phase, dir, { customConfig, rawConfig, silent = true, reportExperimentalFeatures, reactProductionProfiling, debugPrerender, bundler }) {
    ...
    if (path == null ? void 0 : path.length) {
        ...
        try {
            ...
            loadedConfig = Object.freeze(await normalizeConfig(phase, interopDefault(userConfigModule)));
        } catch (err) {
        ...
        if (((_userConfig_experimental3 = userConfig.experimental) == null ? void 0 : _userConfig_experimental3.lightningCssFeatures) && !((_userConfig_experimental4 = userConfig.experimental) == null ? void 0 : _userConfig_experimental4.useLightningcss) && bundler !== Bundler.Turbopack) {
        ...
        if (phase !== PHASE_PRODUCTION_SERVER && ((_userConfig_experimental5 = userConfig.experimental) == null ? void 0 : _userConfig_experimental5.useLightningcss)) {
