...
import { dropDevValidationWorker, mirrorModuleStateToDevValidationWorker } from './dev-validation-worker-pool';
...
import { formatIssue, isFileSystemCacheEnabledForDev, isWellKnownError, ModuleBuildError, processIssues, renderStyledStringToErrorAnsi } from '../../shared/lib/turbopack/utils';
...
/** Output directory (relative to `distDir`) of server-HMR-managed chunks. */ const SERVER_HMR_CHUNKS_DIR = join('server', 'chunks');
/**
...
export async function createHotReloaderTurbopack(opts, serverFields, distDir, resetFetch, lockfile, serverFastRefresh) {
    ...
    setBundlerFindSourceMapImplementation(getSourceMapFromTurbopack.bind(null, project));
            ...
            wsServer.handleUpgrade(req, socket, head, (client)=>{
                ...
                (async function() {
                    ...
                    const syncMessage = {
                        ...
                        devToolsConfig,
                        requestInsights: nextConfig.experimental.requestInsights || isRequestInsightsEnabled() ? getRequestInsightsSnapshot() : undefined
                    };
    ...
    if (serverFastRefresh) {
        setupServerHmr(project, {
            ...
            onApplied: (chunkPaths)=>{
                ...
                const manifestPaths = chunkPaths.map((chunkPath)=>join(distDir, chunkPath));
