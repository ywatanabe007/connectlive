...
function getPprAppPageClassification(route, result) {
    if (typeof (result == null ? void 0 : result.htmlSize) !== 'number') {
        throw Object.defineProperty(new InvariantError(`Expected an HTML size for prerendered app route "${route}"`), "__NEXT_ERROR_CODE", {
            value: "E1456",
            enumerable: false,
            configurable: true
        });
    ...
    if (typeof result.hasEmptyStaticShell !== 'boolean' || typeof result.hasPostponed !== 'boolean' || typeof result.hasPendingUi !== 'boolean') {
        throw Object.defineProperty(new InvariantError(`Expected prerender classification data for PPR route "${route}"`), "__NEXT_ERROR_CODE", {
            value: "E1457",
            enumerable: false,
            configurable: true
        });
...
export default async function build(dir, experimentalAnalyze = false, reactProductionProfiling = false, debugOutput = false, debugPrerender = false, noMangling = false, appDirOnly = false, bundler = Bundler.Turbopack, experimentalBuildMode, traceUploadUrl, debugBuildPathsPatterns, enabledFeatures = {}) {
    ...
    const flushTurbopackWarnings = ()=>{
        if (deferredTurbopackWarnings && deferredTurbopackWarnings.length > 0) {
            console.warn(`${formatWarningsHeader(deferredTurbopackWarnings.length)}\n${deferredTurbopackWarnings.join('\n')}`);
        }
    ...
    try {
        ...
        await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            const debugBuildPaths = debugBuildPathsPatterns ? await (async ()=>{
                const resolved = await resolveBuildPaths(debugBuildPathsPatterns, dir, config.pageExtensions);
                return {
            ...
            await nextBuildSpan.traceChild('generate-route-types').traceAsyncFn(async ()=>{
                ...
                await writeRootParamsTypes(routeTypesManifest, path.join(distDir, 'types', 'root-params.d.ts'));
            });
            ...
            await recordFrameworkVersion("16.3.1");
            ...
            if (!isGenerateMode) {
                if (bundler === Bundler.Turbopack) {
                    const { duration: compilerDuration, shutdownPromise: p, warnings, ...rest } = await turbopackBuild(process.env.NEXT_TURBOPACK_USE_WORKER === undefined || process.env.NEXT_TURBOPACK_USE_WORKER !== '0', telemetry);
                    shutdownPromise = p;
            ...
            if (existsSync(serviceWorkerDir) && (await fs.readdir(serviceWorkerDir)).length > 0) {
                routesManifest.headers.push(buildCustomRoute('header', {
            ...
            if (!isCompileMode && (combinedPages.length > 0 || useStaticPages404 || useDefaultStatic500 || appDir)) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    sortedStaticPaths.forEach(([originalAppPath, prerenderedRoutes])=>{
                        ...
                        // pattern.
                        const htmlLimitedBotsRegexString = config.htmlLimitedBots || HTML_LIMITED_BOT_UA_RE_STRING;
                        ...
                        for (const prerenderedRoute of prerenderedRoutes){
                            ...
                            const staticMetadataPrerenderPathname = getStaticMetadataPrerenderPathname(prerenderedRoute.pathname);
                    ...
                    const updatePagesManifestForExportedPage = (page, isSsg)=>{
                        const isUnusedStaticStatusPage = STATIC_STATUS_PAGES.includes(page) && !usedStaticStatusPages.includes(page);
                        ...
                        if (i18n) {
                            ...
                            for (const locale of i18n.locales){
                                ...
                                const relativeDest = page === '/' ? `pages/${locale}.html` : `pages${normalizePagePath(curPath)}.html`;
                    ...
                    const hasStaticApp404 = hasApp404 && staticPaths.has(UNDERSCORE_NOT_FOUND_ROUTE_ENTRY) && !((_pageInfos_get = pageInfos.get(UNDERSCORE_NOT_FOUND_ROUTE)) == null ? void 0 : _pageInfos_get.hasPostponed);
                    if (hasStaticApp404) {
                            ...
                            if (!isDynamic) {
                                if (i18n) {
                                    ...
                                    for (const locale of i18n.locales){
                                        ...
                                        if (isNotFoundTrue) {
                                            await deleteNotFoundPageFiles(normalizePagePath(localePage));
                                        }
                                ...
                                for (const route of additionalPaths.get(page) ?? []){
                                    const isNotFoundTrue = prerenderManifest.notFoundRoutes.includes(route.pathname);
                                    if (isNotFoundTrue) {
                                        await deleteNotFoundPageFiles(normalizePagePath(route.pathname));
                                    }
                                    const cacheControl = getCacheControl(route.pathname);
                                    prerenderManifest.routes[route.pathname] = {
                                        ...!isNotFoundTrue && {
